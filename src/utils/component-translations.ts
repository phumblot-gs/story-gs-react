import { TranslationMap } from './translations';

// Traductions pour tous les composants UI de la librairie
export const componentTranslations: TranslationMap = {
  // FileBrowser
  "fileBrowser.filterByDate": {
    EN: "Filter by date",
    FR: "Filtrer par date",
    ES: "Filtrar por fecha",
    IT: "Filtra per data",
    DE: "Nach Datum filtern"
  },
  "fileBrowser.refresh": {
    EN: "Refresh",
    FR: "Actualiser",
    ES: "Actualizar",
    IT: "Aggiorna",
    DE: "Aktualisieren"
  },
  "fileBrowser.selected": {
    EN: "{count} selected",
    FR: "{count} sélectionné{plural}",
    ES: "{count} seleccionado{plural}",
    IT: "{count} selezionato{plural}",
    DE: "{count} ausgewählt"
  },
  "fileBrowser.moveTo": {
    EN: "Move to",
    FR: "Déplacer vers",
    ES: "Mover a",
    IT: "Sposta in",
    DE: "Verschieben nach"
  },
  "fileBrowser.download": {
    EN: "Download",
    FR: "Télécharger",
    ES: "Descargar",
    IT: "Scarica",
    DE: "Herunterladen"
  },
  "fileBrowser.share": {
    EN: "Share",
    FR: "Partager",
    ES: "Compartir",
    IT: "Condividi",
    DE: "Teilen"
  },
  "fileBrowser.delete": {
    EN: "Delete",
    FR: "Supprimer",
    ES: "Eliminar",
    IT: "Elimina",
    DE: "Löschen"
  },
  "fileBrowser.rename": {
    EN: "Rename",
    FR: "Renommer",
    ES: "Renombrar",
    IT: "Rinomina",
    DE: "Umbenennen"
  },
  "fileBrowser.columnName": {
    EN: "NAME",
    FR: "NOM",
    ES: "NOMBRE",
    IT: "NOME",
    DE: "NAME"
  },
  "fileBrowser.columnModified": {
    EN: "LAST MODIFIED",
    FR: "DERNIÈRE MODIFICATION",
    ES: "ÚLTIMA MODIFICACIÓN",
    IT: "ULTIMA MODIFICA",
    DE: "ZULETZT GEÄNDERT"
  },
  "fileBrowser.columnSize": {
    EN: "SIZE",
    FR: "TAILLE",
    ES: "TAMAÑO",
    IT: "DIMENSIONE",
    DE: "GRÖSSE"
  },
  "fileBrowser.loading": {
    EN: "Loading...",
    FR: "Chargement...",
    ES: "Cargando...",
    IT: "Caricamento...",
    DE: "Wird geladen..."
  },
  "fileBrowser.empty": {
    EN: "No files in this folder",
    FR: "Aucun fichier dans ce dossier",
    ES: "No hay archivos en esta carpeta",
    IT: "Nessun file in questa cartella",
    DE: "Keine Dateien in diesem Ordner"
  },
  "fileBrowser.dropFiles": {
    EN: "Drop files here to upload",
    FR: "Déposez les fichiers ici pour les téléverser",
    ES: "Suelta los archivos aquí para cargarlos",
    IT: "Rilascia i file qui per caricarli",
    DE: "Dateien hier ablegen zum Hochladen"
  },
  "fileBrowser.dropFilesOr": {
    EN: "or",
    FR: "ou",
    ES: "o",
    IT: "o",
    DE: "oder"
  },
  "fileBrowser.browseFiles": {
    EN: "browse files",
    FR: "parcourir les fichiers",
    ES: "explorar archivos",
    IT: "sfoglia file",
    DE: "Dateien durchsuchen"
  },

  // FolderBrowser
  "folderBrowser.rootFolder": {
    EN: "My files",
    FR: "Mes fichiers",
    ES: "Mis archivos",
    IT: "I miei file",
    DE: "Meine Dateien"
  },
  "folderBrowser.back": {
    EN: "Back",
    FR: "Retour",
    ES: "Atrás",
    IT: "Indietro",
    DE: "Zurück"
  },
  "folderBrowser.empty": {
    EN: "This folder is empty",
    FR: "Ce dossier est vide",
    ES: "Esta carpeta está vacía",
    IT: "Questa cartella è vuota",
    DE: "Dieser Ordner ist leer"
  },
  "folderBrowser.uploadHint": {
    EN: "Drop files here or click to upload",
    FR: "Déposez des fichiers ici ou cliquez pour téléverser",
    ES: "Suelta archivos aquí o haz clic para cargar",
    IT: "Rilascia i file qui o clicca per caricare",
    DE: "Dateien hier ablegen oder klicken zum Hochladen"
  },

  // Button
  "button.loading": {
    EN: "Loading...",
    FR: "Chargement...",
    ES: "Cargando...",
    IT: "Caricamento...",
    DE: "Wird geladen..."
  },
  "button.submit": {
    EN: "Submit",
    FR: "Envoyer",
    ES: "Enviar",
    IT: "Invia",
    DE: "Senden"
  },
  "button.cancel": {
    EN: "Cancel",
    FR: "Annuler",
    ES: "Cancelar",
    IT: "Annulla",
    DE: "Abbrechen"
  },
  "button.save": {
    EN: "Save",
    FR: "Enregistrer",
    ES: "Guardar",
    IT: "Salva",
    DE: "Speichern"
  },
  "button.edit": {
    EN: "Edit",
    FR: "Modifier",
    ES: "Editar",
    IT: "Modifica",
    DE: "Bearbeiten"
  },
  "button.close": {
    EN: "Close",
    FR: "Fermer",
    ES: "Cerrar",
    IT: "Chiudi",
    DE: "Schließen"
  },

  // Select
  "select.placeholder": {
    EN: "Select an option",
    FR: "Sélectionnez une option",
    ES: "Selecciona una opción",
    IT: "Seleziona un'opzione",
    DE: "Option auswählen"
  },
  "select.noOptions": {
    EN: "No options available",
    FR: "Aucune option disponible",
    ES: "No hay opciones disponibles",
    IT: "Nessuna opzione disponibile",
    DE: "Keine Optionen verfügbar"
  },
  "select.clear": {
    EN: "Clear",
    FR: "Effacer",
    ES: "Limpiar",
    IT: "Cancella",
    DE: "Löschen"
  },
  "select.search": {
    EN: "Search...",
    FR: "Rechercher...",
    ES: "Buscar...",
    IT: "Cerca...",
    DE: "Suchen..."
  },

  // Modal
  "modal.close": {
    EN: "Close",
    FR: "Fermer",
    ES: "Cerrar",
    IT: "Chiudi",
    DE: "Schließen"
  },
  "modal.confirm": {
    EN: "Confirm",
    FR: "Confirmer",
    ES: "Confirmar",
    IT: "Conferma",
    DE: "Bestätigen"
  },
  "modal.cancel": {
    EN: "Cancel",
    FR: "Annuler",
    ES: "Cancelar",
    IT: "Annulla",
    DE: "Abbrechen"
  },

  // Pagination
  "pagination.previous": {
    EN: "Previous",
    FR: "Précédent",
    ES: "Anterior",
    IT: "Precedente",
    DE: "Zurück"
  },
  "pagination.next": {
    EN: "Next",
    FR: "Suivant",
    ES: "Siguiente",
    IT: "Successivo",
    DE: "Weiter"
  },
  "pagination.page": {
    EN: "Page {current} of {total}",
    FR: "Page {current} sur {total}",
    ES: "Página {current} de {total}",
    IT: "Pagina {current} di {total}",
    DE: "Seite {current} von {total}"
  },

  // Toast/Notifications
  "toast.success": {
    EN: "Success",
    FR: "Succès",
    ES: "Éxito",
    IT: "Successo",
    DE: "Erfolg"
  },
  "toast.error": {
    EN: "Error",
    FR: "Erreur",
    ES: "Error",
    IT: "Errore",
    DE: "Fehler"
  },
  "toast.warning": {
    EN: "Warning",
    FR: "Avertissement",
    ES: "Advertencia",
    IT: "Avviso",
    DE: "Warnung"
  },
  "toast.info": {
    EN: "Information",
    FR: "Information",
    ES: "Información",
    IT: "Informazione",
    DE: "Information"
  },

  // Date filters (FileBrowser)
  "dateFilter.all": {
    EN: "All",
    FR: "Tous",
    ES: "Todos",
    IT: "Tutti",
    DE: "Alle"
  },
  "dateFilter.today": {
    EN: "Today",
    FR: "Aujourd'hui",
    ES: "Hoy",
    IT: "Oggi",
    DE: "Heute"
  },
  "dateFilter.week": {
    EN: "This week",
    FR: "Cette semaine",
    ES: "Esta semana",
    IT: "Questa settimana",
    DE: "Diese Woche"
  },
  "dateFilter.month": {
    EN: "This month",
    FR: "Ce mois",
    ES: "Este mes",
    IT: "Questo mese",
    DE: "Diesen Monat"
  },
  "dateFilter.year": {
    EN: "This year",
    FR: "Cette année",
    ES: "Este año",
    IT: "Quest'anno",
    DE: "Dieses Jahr"
  },

  // Common actions
  "actions.upload": {
    EN: "Upload",
    FR: "Téléverser",
    ES: "Cargar",
    IT: "Carica",
    DE: "Hochladen"
  },
  "actions.create": {
    EN: "Create",
    FR: "Créer",
    ES: "Crear",
    IT: "Crea",
    DE: "Erstellen"
  },
  "actions.update": {
    EN: "Update",
    FR: "Mettre à jour",
    ES: "Actualizar",
    IT: "Aggiorna",
    DE: "Aktualisieren"
  },
  "actions.remove": {
    EN: "Remove",
    FR: "Retirer",
    ES: "Eliminar",
    IT: "Rimuovi",
    DE: "Entfernen"
  },
  "actions.copy": {
    EN: "Copy",
    FR: "Copier",
    ES: "Copiar",
    IT: "Copia",
    DE: "Kopieren"
  },
  "actions.paste": {
    EN: "Paste",
    FR: "Coller",
    ES: "Pegar",
    IT: "Incolla",
    DE: "Einfügen"
  },
  "actions.cut": {
    EN: "Cut",
    FR: "Couper",
    ES: "Cortar",
    IT: "Taglia",
    DE: "Ausschneiden"
  },

  // Status messages
  "status.loading": {
    EN: "Loading...",
    FR: "Chargement...",
    ES: "Cargando...",
    IT: "Caricamento...",
    DE: "Wird geladen..."
  },
  "status.saving": {
    EN: "Saving...",
    FR: "Enregistrement...",
    ES: "Guardando...",
    IT: "Salvataggio...",
    DE: "Wird gespeichert..."
  },
  "status.saved": {
    EN: "Saved",
    FR: "Enregistré",
    ES: "Guardado",
    IT: "Salvato",
    DE: "Gespeichert"
  },
  "status.error": {
    EN: "An error occurred",
    FR: "Une erreur s'est produite",
    ES: "Se produjo un error",
    IT: "Si è verificato un errore",
    DE: "Ein Fehler ist aufgetreten"
  },
  "status.success": {
    EN: "Operation successful",
    FR: "Opération réussie",
    ES: "Operación exitosa",
    IT: "Operazione riuscita",
    DE: "Vorgang erfolgreich"
  },

  // Validation messages
  "validation.required": {
    EN: "This field is required",
    FR: "Ce champ est requis",
    ES: "Este campo es obligatorio",
    IT: "Questo campo è obbligatorio",
    DE: "Dieses Feld ist erforderlich"
  },
  "validation.email": {
    EN: "Please enter a valid email",
    FR: "Veuillez entrer un email valide",
    ES: "Por favor ingrese un email válido",
    IT: "Inserisci un'email valida",
    DE: "Bitte geben Sie eine gültige E-Mail ein"
  },
  "validation.minLength": {
    EN: "Minimum {min} characters required",
    FR: "Minimum {min} caractères requis",
    ES: "Se requieren mínimo {min} caracteres",
    IT: "Minimo {min} caratteri richiesti",
    DE: "Mindestens {min} Zeichen erforderlich"
  },
  "validation.maxLength": {
    EN: "Maximum {max} characters allowed",
    FR: "Maximum {max} caractères autorisés",
    ES: "Máximo {max} caracteres permitidos",
    IT: "Massimo {max} caratteri consentiti",
    DE: "Maximal {max} Zeichen erlaubt"
  }
};